...
checkCentosSELinux() {
    if [[ -f "/etc/selinux/config" ]] && ! grep -q "SELINUX=disabled" <"/etc/selinux/config"; then
        ...
        echoContent yellow "https://www.v2a.example.com/archives/1684115970026#centos7-%E5%85%B3%E9%97%ADselinux"
        exit 0
    fi
}
checkSystem() {
...
acmeInstallSSL() {
    ...
    if [[ "${dnsAPIType}" == "cloudflare" ]]; then
        ...
        sudo CF_Token="${cfAPIToken}" "$HOME/.acme.sh/acme.sh" --issue -d "${dnsAPIDomain}" -d "${dnsTLSDomain}" --dns dns_cf -k ec-256 --server "${sslType}" ${sslIPv6} 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null
    elif [[ "${dnsAPIType}" == "aliyun" ]]; then
        echoContent green " --->  DNS API 生成证书中"
        sudo Ali_Key="${aliKey}" Ali_Secret="${aliSecret}" "$HOME/.acme.sh/acme.sh" --issue -d "${dnsAPIDomain}" -d "${dnsTLSDomain}" --dns dns_ali -k ec-256 --server "${sslType}" ${sslIPv6} 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null
    else
...
installV2Ray() {
    ...
    if [[ "${coreInstallType}" != "2" && "${coreInstallType}" != "3" ]]; then
        ...
        if [[ "${release}" == "alpine" ]]; then
            wget -c -q -P /etc/v2a/v2ray/ "https://github.com/v2fly/v2ray-core/releases/download/${version}/${v2rayCoreCPUVendor}.zip"
        else
        ...
        if [[ "${selectCoreType}" == "3" ]]; then
            ...
            echoContent green " ---> v2ray-core版本:$(/etc/v2a/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installSingBox() {
    ...
    if [[ ! -f "/etc/v2a/sing-box/sing-box" ]]; then
        ...
        echoContent green " ---> sing-box版本:v$(/etc/v2a/sing-box/sing-box version | grep "sing-box version" | awk '{print $3}')"
        if [[ -z "${lastInstallationConfig}" ]]; then
...
updateV2Ray() {
    ...
    if [[ -z "${coreInstallType}" ]]; then
        ...
        echoContent green " ---> 当前v2ray-core版本:$(/etc/v2a/v2ray/v2ray --version | awk '{print $2}' | head -1)"
...
installHysteriaService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2a/hysteria/hysteria server -c /etc/v2a/hysteria/conf/config.json --log-level debug'
...
handleTuic() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "请手动执行【/etc/v2a/tuic/tuic -c /etc/v2a/tuic/conf/config.json】，查看错误日志"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
unInstallXrayCoreReality() {
    ...
    rm /etc/v2a/xray/conf/08_VLESS_vision_gRPC_inbounds.json
    echoContent green " ---> 卸载完成"
}
...
initRealityMldsa65() {
    ...
    if /etc/v2a/xray/xray tls ping "${realityServerName}:${realityDomainPort}" 2>/dev/null | grep -q "X25519MLKEM768"; then
        length=$(/etc/v2a/xray/xray tls ping "${realityServerName}:${realityDomainPort}" | grep "Certificate chain's total length:" | awk '{print $5}')
...
menu() {
    ...
    echoContent red "\n=========================== 推广区============================"
    echoContent red "                                              "
    echoContent green "VPS选购攻略：https://www.v2a.example.com/archives/1679975663984"
    echoContent green "年付10美金低价VPS AS4837：https://www.v2a.example.com/archives/racknerdtao-can-zheng-li-nian-fu-10mei-yuan"
    echoContent green "优质常驻套餐DMIT CN2-GIA：https://www.v2a.example.com/archives/186cee7b-9459-4e57-b9b2-b07a4f36931c"
    echoContent green "VPS探针：https://ping.v2a.example.com/"
    echoContent red "=============================================================="
    if [[ -n "${coreInstallType}" ]]; then
        echoContent yellow "1.重新安装"
    else
