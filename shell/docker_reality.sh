...
showHelp() {
    ...
    echoContent "white" "  1. 交互模式会同时检测 config.json 与 v2a-docker 容器是否存在，再显示对应菜单。"
    ...
    echoContent "white" "  --data-dir /tmp/v2a/docker  隔离验证的路径覆盖示例"
    exit "${1:-0}"
}
...
# checkPortInUse — 若端口已被 v2a-docker 容器以外的进程占用则退出
# （与 install.sh:1951-1957 保持一致）。
# 重新运行时脚本会在重建前执行 docker rm -f v2a-docker，
...
checkPortInUse() {
    ...
    if [[ "${containerExists}" == "1" ]]; then
        ...
        if [[ "${unrelatedPids}" == "0" ]]; then
            # 端口仅由 v2a-docker 的 docker-proxy 占用 — 可以继续
            echoContent "yellow" "端口 ${p} 由现有 v2a-docker 容器占用（将被替换）"
            return 0
        fi
    fi
