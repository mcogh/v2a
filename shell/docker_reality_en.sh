...
showQRCode() {
    ...
    if command -v qrencode >/dev/null 2>&1; then
        ...
        echoContent "yellow" " ---> qrencode is not installed; copy the link and generate a QR code manually"
    fi
}
...
showHelp() {
    ...
    echoContent "white" "  1. Interactive mode checks both config.json and the v2a-docker container before showing a menu."
    ...
    echoContent "white" "  --data-dir /tmp/v2a/docker  Example path override for isolated verification"
    exit "${1:-0}"
}
...
startSubscribeContainer() {
    local displayAddress="$1"
    local containerName="v2a-docker-subscribe"
...
# checkPortInUse — exit if the port is occupied by a process other than the v2a-docker container
# (aligned with install.sh:1951-1957).
# On re-run the script does docker rm -f v2a-docker before recreating,
...
checkPortInUse() {
    ...
    # Check whether the existing v2a-docker container holds the port.
    ...
    if docker ps -a --filter "name=^/v2a-docker$" --format '{{.Names}}' 2>/dev/null \
            | grep -q "^v2a-docker$" || docker ps -a --filter "name=^/v2a-docker-subscribe$" --format '{{.Names}}' 2>/dev/null | grep -q "^v2a-docker-subscribe$"; then
        containerExists=1
    fi
    ...
    if [[ "${containerExists}" == "1" ]]; then
        ...
        if [[ "${unrelatedPids}" == "0" ]]; then
            # Port is held only by v2a-docker's docker-proxy — safe to continue
            echoContent "yellow" "Port ${p} is held by the existing v2a-docker container (will be replaced)"
            return 0
        fi
    fi
...
loadPersistedSummaryIfPresent() {
    ...
    if [[ -n "${persistedContainer}" && "${persistedContainer}" != "v2a-docker" ]]; then
...
hasManagedContainer() {
    docker ps -a --filter "name=^/v2a-docker$" --format '{{.Names}}' 2>/dev/null | grep -q '^v2a-docker$'
}
...
uninstallDockerReality() {
    ...
    *)
    ...
    if hasManagedContainer; then
        docker rm -f v2a-docker >/dev/null 2>&1 || true
        echoContent "green" " ---> Docker container removed"
    fi
    if docker ps -a --filter "name=^/v2a-docker-subscribe$" --format '{{.Names}}' 2>/dev/null | grep -q '^v2a-docker-subscribe$'; then
        docker rm -f v2a-docker-subscribe >/dev/null 2>&1 || true
...
startContainer() {
    local xrayImage="ghcr.io/xtls/xray-core:26.5.9"
    local containerName="v2a-docker"
