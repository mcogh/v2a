...
readInstallType() {
    ...
    if [[ -d "/etc/v2a" ]]; then
        ...
        if [[ -d "/etc/v2a/v2ray" && -f "/etc/v2a/v2ray/v2ray" && -f "/etc/v2a/v2ray/v2ctl" ]]; then
            if [[ -d "/etc/v2a/v2ray/conf" && -f "/etc/v2a/v2ray/conf/02_VLESS_TCP_inbounds.json" ]]; then
                configPath=/etc/v2a/v2ray/conf/
                if grep </etc/v2a/v2ray/conf/02_VLESS_TCP_inbounds.json -q '"security": "tls"'; then
                    coreInstallType=2
                    ctlPath=/etc/v2a/v2ray/v2ctl
                fi
            fi
        fi

        if [[ -d "/etc/v2a/xray" && -f "/etc/v2a/xray/xray" ]]; then
            # Detect xray-core here
            if [[ -d "/etc/v2a/xray/conf" ]] && [[ -f "/etc/v2a/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/v2a/xray/conf/02_trojan_TCP_inbounds.json" || -f "/etc/v2a/xray/conf/07_VLESS_vision_reality_inbounds.json" ]]; then
                #xray-core
        ...
        if [[ -d "/etc/v2a/hysteria" && -f "/etc/v2a/hysteria/hysteria" ]]; then
            # Detect hysteria here
            if [[ -d "/etc/v2a/hysteria/conf" ]] && [[ -f "/etc/v2a/hysteria/conf/config.json" ]] && [[ -f "/etc/v2a/hysteria/conf/client_network.json" ]]; then
                hysteriaConfigPath=/etc/v2a/hysteria/conf/
            fi
        fi

        if [[ -d "/etc/v2a/tuic" && -f "/etc/v2a/tuic/tuic" ]]; then
            if [[ -d "/etc/v2a/tuic/conf" ]] && [[ -f "/etc/v2a/tuic/conf/config.json" ]]; then
                tuicConfigPath=/etc/v2a/tuic/conf/
            fi
        fi
    fi
}
...
allowPort() {
    ...
    if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
        ...
        if ! iptables -L | grep -q "$1/${type}(mcogh)"; then
            updateFirewalldStatus=true
            iptables -I INPUT -p ${type} --dport "$1" -m comment --comment "allow $1/${type}(mcogh)" -j ACCEPT
        fi
...
cleanUp() {
    if [[ "$1" == "v2rayClean" ]]; then
        rm -rf "$(find /etc/v2a/v2ray/* | grep -E '(config_full.json|conf)')"
        handleV2Ray stop >/dev/null
        rm -f /etc/systemd/system/v2ray.service
    elif [[ "$1" == "xrayClean" ]]; then
        rm -rf "$(find /etc/v2a/xray/* | grep -E '(config_full.json|conf)')"
...
mkdirTools() {
    ...
    mkdir -p /etc/v2a/v2ray/conf
    mkdir -p /etc/v2a/v2ray/tmp
    mkdir -p /etc/v2a/xray/conf
    mkdir -p /etc/v2a/xray/tmp
    mkdir -p /etc/v2a/hysteria/conf
    mkdir -p /etc/systemd/system/
    mkdir -p /tmp/v2a-tls/

    mkdir -p /etc/v2a/warp

    mkdir -p /etc/v2a/tuic/conf
}
...
acmeInstallSSL() {
    if [[ "${dnsSSLStatus}" == "true" ]]; then
        ...
        sudo "$HOME/.acme.sh/acme.sh" --issue -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --dns --yes-I-know-dns-manual-mode-enough-go-ahead-please -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null

        local txtValue=
        txtValue=$(tail -n 10 /etc/v2a/tls/acme.log | grep "TXT value" | awk -F "'" '{print $2}')
        if [[ -n "${txtValue}" ]]; then
            echoContent green " ---> Please add DNS TXT record manually"
            echoContent yellow " ---> Please refer to this tutorial for adding method, https://github.com/mcogh/v2a/blob/master/documents/dns_txt.md"
            ...
            if [[ "${addDNSTXTRecordStatus}" == "y" ]]; then
                ...
                if echo "${txtAnswer}" | grep -q "^${txtValue}"; then
                    ...
                    if [[ -n "${installSSLIPv6}" ]]; then
                        sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null
                    else
                        sudo "$HOME/.acme.sh/acme.sh" --renew -d "*.${dnsTLSDomain}" -d "${dnsTLSDomain}" --yes-I-know-dns-manual-mode-enough-go-ahead-please --ecc --server "${sslType}" 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null
                    fi
                else
        ...
        echoContent green " ---> Generating certificate"
        sudo "$HOME/.acme.sh/acme.sh" --issue -d "${tlsDomain}" --standalone -k ec-256 --server "${sslType}" ${installSSLIPv6} 2>&1 | tee -a /etc/v2a/tls/acme.log >/dev/null
    fi
}
# Custom port
customPortFunction() {
...
installTLS() {
    ...
    # Install tls
    if [[ -f "/etc/v2a/tls/${tlsDomain}.crt" && -f "/etc/v2a/tls/${tlsDomain}.key" && -n $(cat "/etc/v2a/tls/${tlsDomain}.crt") ]] || [[ -d "$HOME/.acme.sh/${tlsDomain}_ecc" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.key" && -f "$HOME/.acme.sh/${tlsDomain}_ecc/${tlsDomain}.cer" ]]; then
...
nginxBlog() {
    ...
    if [[ -d "${nginxStaticPath}" && -f "${nginxStaticPath}/check" ]]; then
        ...
        if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
            ...
            wget -q -P "${nginxStaticPath}" https://raw.githubusercontent.com/mcogh/v2a/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
installHysteria() {
    ...
    if [[ -z "${hysteriaConfigPath}" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2a/hysteria/ "https://github.com/apernet/hysteria/releases/download/${version}/${hysteriaCoreCPUVendor}"
        mv "/etc/v2a/hysteria/${hysteriaCoreCPUVendor}" /etc/v2a/hysteria/hysteria
        chmod 655 /etc/v2a/hysteria/hysteria
    else
        echoContent green " ---> Hysteria version:$(/etc/v2a/hysteria/hysteria --version | awk '{print $3}')"
        read -r -p "Would you like to update or upgrade? [y/n]:" reInstallHysteriaStatus
        if [[ "${reInstallHysteriaStatus}" == "y" ]]; then
            rm -f /etc/v2a/hysteria/hysteria
            installHysteria "$1"
        fi
    fi
...
installTuic() {
    ...
    if [[ -z "${tuicConfigPath}" ]]; then
        ...
        wget -c -q "${wgetShowProgressStatus}" -P /etc/v2a/tuic/ "https://github.com/EAimTY/tuic/releases/download/${version}/${version}${tuicCoreCPUVendor}"
        mv "/etc/v2a/tuic/${version}${tuicCoreCPUVendor}" /etc/v2a/tuic/tuic
        chmod 655 /etc/v2a/tuic/tuic
    else
        echoContent green " ---> Tuic version:$(/etc/v2a/tuic/tuic -v)"
        read -r -p "Would you like to update or upgrade? [y/n]:" reInstallTuicStatus
        if [[ "${reInstallTuicStatus}" == "y" ]]; then
            rm -f /etc/v2a/tuic/tuic
            tuicConfigPath=
            installTuic "$1"
        fi
    fi
...
installV2RayService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2a/v2ray/v2ray -confdir /etc/v2a/v2ray/conf'
...
installHysteriaService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2a/hysteria/hysteria --log-level info -c /etc/v2a/hysteria/conf/config.json server'
...
installTuicService() {
    ...
    if [[ -n $(find /bin /usr/bin -name "systemctl") ]]; then
        ...
        execStart='/etc/v2a/tuic/tuic -c /etc/v2a/tuic/conf/config.json'
...
handleV2Ray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "v2ray/v2ray") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2a/v2ray/v2ray -confdir /etc/v2a/v2ray/conf] and check the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleHysteria() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "hysteria/hysteria") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2a/hysteria/hysteria --log-level debug -c /etc/v2a/hysteria/conf/config.json server] to view the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleTuic() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "tuic/tuic") ]]; then
            ...
            echoContent red "Please manually execute [/etc/v2a/tuic/tuic -c /etc/v2a/tuic/conf/config.json] and check the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
handleXray() {
    ...
    if [[ "$1" == "start" ]]; then
        if [[ -n $(pgrep -f "xray/xray") ]]; then
            ...
            echoContent red "Please manually execute the following command [/etc/v2a/xray/xray -confdir /etc/v2a/xray/conf] and feedback the error log"
            exit 0
        fi
    elif [[ "$1" == "stop" ]]; then
...
initHysteriaNetwork() {
    ...
    cat <<EOF >/etc/v2a/hysteria/conf/client_network.json
{
...
hysteriaPortHopping() {
    ...
    if [[ -z "${hysteriaPortHoppingRange}" ]]; then
        ...
        if [[ -z "${portStart}" || -z "${portEnd}" ]]; then
            ...
            # hysteriaPortHopping
            #else
            iptables -t nat -A PREROUTING -p udp --dport "${portStart}:${portEnd}" -m comment --comment "mcogh_portHopping" -j DNAT --to-destination :${hysteriaPort}

            if iptables-save | grep -q "mcogh_portHopping"; then
...
readHysteriaPortHopping() {
    if [[ -n "${hysteriaPort}" ]]; then
        ...
        if iptables-save | grep -q "mcogh_portHopping"; then
            portHopping=
            portHopping=$(iptables-save | grep "mcogh_portHopping" | cut -d " " -f 8)
...
deleteHysteriaPortHoppingRules() {
    iptables -t nat -L PREROUTING --line-numbers | grep "mcogh_portHopping" | awk '{print $1}' | while read -r line; do
...
initHysteriaConfig() {
    ...
    cat <<EOF >/etc/v2a/hysteria/conf/config.json
{
	"listen": ":${hysteriaPort}",
	"protocol": "${hysteriaProtocol}",
	"disable_udp": false,
	"cert": "/etc/v2a/tls/${currentHost}.crt",
	"key": "/etc/v2a/tls/${currentHost}.key",
	"auth": {
		"mode": "passwords",
		"config": []
	},
...
EOF
    ...
    addClientsHysteria "/etc/v2a/hysteria/conf/config.json" true
...
initV2RayConfig() {
    ...
    movePreviousConfig
    # log
    cat <<EOF >/etc/v2a/v2ray/conf/00_log.json
{
  "log": {
    "error": "/etc/v2a/v2ray/error.log",
    "loglevel": "warning"
  }
}
EOF
    # outbounds
    if [[ -n "${pingIPv6}" ]]; then
        cat <<EOF >/etc/v2a/v2ray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
        ...
        cat <<EOF >/etc/v2a/v2ray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
    ...
    # dns
    cat <<EOF >/etc/v2a/v2ray/conf/11_dns.json
{
    "dns": {
        "servers": [
          "localhost"
...
EOF
    ...
    if echo "${selectCustomInstallType}" | grep -q 4 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2a/v2ray/conf/04_trojan_TCP_inbounds.json
{
"inbounds":[
	{
	  "port": 31296,
...
EOF
        addClients "/etc/v2a/v2ray/conf/04_trojan_TCP_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 1 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2a/v2ray/conf/03_VLESS_WS_inbounds.json
{
"inbounds":[
    {
	  "port": 31297,
...
EOF
        addClients "/etc/v2a/v2ray/conf/03_VLESS_WS_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2a/v2ray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
        addClients "/etc/v2a/v2ray/conf/04_trojan_gRPC_inbounds.json" "${addClientsStatus}"
    fi
    ...
    if echo "${selectCustomInstallType}" | grep -q 3 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2a/v2ray/conf/05_VMess_WS_inbounds.json
{
"inbounds":[
{
  "listen": "127.0.0.1",
...
EOF
        addClients "/etc/v2a/v2ray/conf/05_VMess_WS_inbounds.json" "${addClientsStatus}"
    fi

    if echo "${selectCustomInstallType}" | grep -q 5 || [[ "$1" == "all" ]]; then
        getClients "${configPath}../tmp/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
        cat <<EOF >/etc/v2a/v2ray/conf/06_VLESS_gRPC_inbounds.json
{
    "inbounds":[
    {
        "port": 31301,
...
EOF
        addClients "/etc/v2a/v2ray/conf/06_VLESS_gRPC_inbounds.json" "${addClientsStatus}"
    fi
    ...
    cat <<EOF >/etc/v2a/v2ray/conf/02_VLESS_TCP_inbounds.json
{
"inbounds":[
{
  "port": ${defaultPort},
...
EOF
    addClients "/etc/v2a/v2ray/conf/02_VLESS_TCP_inbounds.json" "${addClientsStatus}"
...
EOF
    ...
    # outbounds
    if [[ ! -f "/etc/v2a/xray/conf/10_ipv6_outbounds.json" ]]; then
        if [[ -n "${pingIPv6}" ]]; then
            cat <<EOF >/etc/v2a/xray/conf/10_ipv6_outbounds.json
{
    "outbounds": [
        {
          "protocol": "freedom",
...
EOF
            ...
            cat <<EOF >/etc/v2a/xray/conf/10_ipv4_outbounds.json
{
    "outbounds":[
        {
            "protocol":"freedom",
...
EOF
    ...
    # routing
    if [[ ! -f "/etc/v2a/xray/conf/09_routing.json" ]]; then
...
EOF
    ...
    if echo "${selectCustomInstallType}" | grep -q 2 || [[ "$1" == "all" ]]; then
        ...
        cat <<EOF >/etc/v2a/xray/conf/04_trojan_gRPC_inbounds.json
{
    "inbounds": [
        {
            "port": 31304,
...
EOF
        ...
        rm /etc/v2a/xray/conf/04_trojan_gRPC_inbounds.json >/dev/null 2>&1
    fi
...
EOF
        ...
        cat <<EOF >/etc/v2a/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json
{
  "inbounds": [
    {
      "port": 31305,
...
EOF
        ...
        rm /etc/v2a/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json >/dev/null 2>&1
    fi
    installSniffing
}
...
EOF
        ...
        cat <<EOF >"/etc/v2a/hysteria/conf/client.json"
{
...
EOF
        local v2rayNConf=
        v2rayNConf="$(cat /etc/v2a/hysteria/conf/client.json)"
...
EOF
        ...
        cat <<EOF >"/etc/v2a/tuic/conf/v2rayN.json"
{
    "relay": {
...
EOF
        local v2rayNConf=
        v2rayNConf="$(cat /etc/v2a/tuic/conf/v2rayN.json)"
        echoContent green "${v2rayNConf}"

        cat <<EOF >>"/etc/v2a/subscribe_local/clashMeta/${email}"
  - name: "${email}_tuic"
    server: ${currentHost}
    type: tuic
    port: ${tuicPort}
...
updateNginxBlog() {
    ...
    if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
        ...
        wget -q -P "${nginxStaticPath}" "https://raw.githubusercontent.com/mcogh/v2a/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
...
unInstall() {
    ...
    #rm -rf /tmp/v2a-tls/*
    # if [[ -d "/etc/v2a/tls" ]] && [[ -n $(find /etc/v2a/tls/ -name "*.key") ]] && [[ -n $(find /etc/v2a/tls/ -name "*.crt") ]]; then
    # mv /etc/v2a/tls /tmp/v2a-tls
    # if [[ -n $(find /tmp/v2a-tls -name '*.key') ]]; then
    # echoContent yellow " ---> Backup certificate successful, please save it. [/tmp/v2a-tls]"
    #fi
    #fi
    ...
    echoContent green " ---> Uninstall v2a script completed"
}
...
customUUID() {
    ...
    if [[ -z "${currentCustomUUID}" ]]; then
        ...
        if [[ -f "/tmp/v2a" && -n $(cat /tmp/v2a) ]]; then
...
unInstallXrayCoreReality() {
    ...
    handleXray stop
    rm /etc/v2a/xray/conf/07_VLESS_vision_reality_inbounds.json
    rm /etc/v2a/xray/conf/08_VLESS_reality_fallback_grpc_inbounds.json
    echoContent green " ---> Uninstall completed"
}
...
cronFunction() {
    if [[ "${cronName}" == "RenewTLS" ]]; then
        ...
        echoContent green " ---> geo update date: $(date "+%F %H:%M:%S")" >>/etc/v2a/crontab_updateGeoSite.log
        exit 0
    fi
}
#Account management
...
addSubscribeMenu() {
    ...
    if [[ "${addSubscribeStatus}" == "1" ]]; then
        ...
        rm -rf /etc/v2a/subscribe_remote/clashMeta/*
        rm -rf /etc/v2a/subscribe_remote/default/*
        echo >/etc/v2a/subscribe_remote/remoteSubscribeUrl
...
addOtherSubscribe() {
    echoContent yellow "#Notes:"
    echoContent yellow "Please read the following article carefully: https://www.v2a.example.com/archives/1681804748677"
    echoContent skyBlue "Input example: www.v2a.example.com:443:vps1\n"
    ...
    if [[ -z "${remoteSubscribeUrl}" ]]; then
        ...
        if [[ -n $(ls /etc/v2a/subscribe/clashMeta/) || -n $(ls /etc/v2a/subscribe/default/) ]]; then
            find /etc/v2a/subscribe_local/default/* | while read -r email; do
                email=$(echo "${email}" | awk -F "[d][e][f][a][u][l][t][/]" '{print $2}')

                local emailMd5=
                emailMd5=$(echo -n "${email}$(cat "/etc/v2a/subscribe_local/subscribeSalt")"$'\n' | md5sum | awk '{print $1}')
                ...
                if echo "${default}" | grep -q "${email}"; then
                    ...
                    echo "${default}" >>"/etc/v2a/subscribe_remote/default/${email}"
                ...
                if echo "${clashMetaProxies}" | grep -q "${email}"; then
                    ...
                    echo "${clashMetaProxies}" >>"/etc/v2a/subscribe_remote/clashMeta/${email}"
    ...
    if echo "${currentInstallProtocolType}" | grep -q 0 && [[ -n "${configPath}" ]]; then
        ...
        if [[ -n $(ls /etc/v2a/subscribe_local/default/) ]]; then
            find /etc/v2a/subscribe_local/default/* | while read -r email; do
                ...
                if [[ -f "/etc/v2a/subscribe_remote/default/${email}" ]]; then
                    echo >"/etc/v2a/subscribe_remote/default/${email}_tmp"
                    while read -r remoteUrl; do
                        updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "default"
                    done < <(grep "VLESS_TCP/TLS_Vision" <"/etc/v2a/subscribe_remote/default/${email}" | awk -F "@" '{print $2}' | awk -F "?" '{print $1}')

                    echo >"/etc/v2a/subscribe_remote/default/${email}"
                    cat "/etc/v2a/subscribe_remote/default/${email}_tmp" >"/etc/v2a/subscribe_remote/default/${email}"
                    cat "/etc/v2a/subscribe_remote/default/${email}" >>"/etc/v2a/subscribe/default/${emailMd5}"
                fi
                ...
                if [[ -f "/etc/v2a/subscribe_local/clashMeta/${email}" ]]; then
                    ...
                    if [[ -f "/etc/v2a/subscribe_remote/clashMeta/${email}" ]]; then
                        echo >"/etc/v2a/subscribe_remote/clashMeta/${email}_tmp"
                        while read -r remoteUrl; do
                            updateRemoteSubscribe "${emailMd5}" "${email}" "${remoteUrl}" "ClashMeta"
                        done < <(grep -A3 "VLESS_TCP/TLS_Vision" <"/etc/v2a/subscribe_remote/clashMeta/${email}" | awk '/server:|port:/ {print $2}' | paste -d ':' - -)
                        echo >"/etc/v2a/subscribe_remote/clashMeta/${email}"
                        cat "/etc/v2a/subscribe_remote/clashMeta/${email}_tmp" >"/etc/v2a/subscribe_remote/clashMeta/${email}"
                        cat "/etc/v2a/subscribe_remote/clashMeta/${email}" >>"/etc/v2a/subscribe/clashMeta/${emailMd5}"
                    fi
...
updateRemoteSubscribe() {
    ...
    local serverAlias=
    serverAlias=$(grep "${remoteDomain}" <"/etc/v2a/subscribe_remote/remoteSubscribeUrl" | awk -F ":" '{print $3}')

    if [[ "${type}" == "ClashMeta" ]]; then
        local clashMetaProxies=
        clashMetaProxies=$(curl -s -4 "https://${remoteUrl}/s/clashMeta/${emailMD5}" | sed '/proxies:/d' | sed "s/${email}/${email}_${serverAlias}/g")
        if echo "${clashMetaProxies}" | grep -q "${email}"; then
            echo "${clashMetaProxies}" >>"/etc/v2a/subscribe_remote/clashMeta/${email}_tmp"
        ...
        default=$(curl -s -4 "https://${remoteUrl}/s/default/${emailMD5}" | base64 -d | sed "s/${email}/${email}_${serverAlias}/g")
        if echo "${default}" | grep -q "${email}"; then
            echo "${default}" >>"/etc/v2a/subscribe_remote/default/${email}_tmp"
...
initRealityDest() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent green "Fallback domain name list: https://www.v2a.example.com/archives/1680104902581#heading-8\n"
...
initRealityClientServersName() {
    if [[ -n "${domain}" ]]; then
        ...
        echoContent yellow "#Notes"
        echoContent green "List of serverNames available to the client: https://www.v2a.example.com/archives/1680104902581#heading-8\n"
...
menu() {
    ...
    echoContent green "VPS purchasing guide: https://www.v2a.example.com/archives/1679975663984"
    echoContent green "Low-price VPS AS4837 with an annual payment of 10 US dollars: https://www.v2a.example.com/archives/racknerdtao-can-zheng-li-nian-fu-10mei-yuan"
    echoContent red "================================================== ==============="
    if [[ -n "${coreInstallType}" ]]; then
        echoContent yellow "1.Reinstall"
    else
