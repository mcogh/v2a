...
readInstallType() {
    ...
    if [[ -d "/etc/v2a" ]]; then
        if [[ -f "/etc/v2a/xray/xray" ]]; then
            if [[ -d "/etc/v2a/xray/conf" ]] && [[ -f "/etc/v2a/xray/conf/02_VLESS_TCP_inbounds.json" || -f "/etc/v2a/xray/conf/02_trojan_TCP_inbounds.json" || -f "/etc/v2a/xray/conf/07_VLESS_vision_reality_inbounds.json" || -f "/etc/v2a/xray/conf/12_VLESS_XHTTP_inbounds.json" ]]; then
                # xray-core
                #xray-core
...
setSniffRouting() {
    local singBoxDNSConfigPath="/etc/v2a/sing-box/conf/config/dns.json"
